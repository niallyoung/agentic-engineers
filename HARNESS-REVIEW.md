...
2. **Claude Code harness is solid but uses a simplified model-mapping strategy** (haiku/sonnet/opus tier names rather than fully-qualified IDs), which may cause issues as Claude Code evolves.
3. **Copilot CLI harness is intentionally minimal** — skills only, no agents. This is correct per the platform's capabilities, but the documentation does not make this limitation prominent enough.
4. **π.dev harness has the most significant issues**: source files in `renderer/pi-dev-src/` are partially stale (old model IDs, 9 roles vs. the canonical 8, references to non-existent pi.dev sub-agent features), and the renderer itself does not transform content — it simply copies files verbatim.
5. **src/ directory structure is clean** for the 8 canonical agents and 14 skills with `SKILL.md`, but `src/skills/` contains substantial non-skill content (loose `.md` files, `.py` files, orphaned agent definitions) that all three Bash harnesses correctly skip.

**Overall compliance:** OpenCode ✅ High | Claude Code ✅ Medium-High | Copilot CLI ✅ Medium | π.dev ⚠️ Low-Medium
...
| Content matches canonical model | ❌ | Stale roles, stale model IDs |
| SPEC.md compliance | ⚠️ | Not verified — pi.dev harness predates SPEC.md |
...
| SPEC.md compliance | ✅ | Uses canonical src/ layout |
| Documentation quality | ⚠️ | No dedicated install guide; docs/INSTALL.md is stale |
...
|---------|-------|----------|-------------|-------------|
| Agents rendered | ⚠️ Static | ✅ Dynamic | ✅ Dynamic | ⚠️ Legacy only |
...
| Model mapping | ⚠️ Stale | ✅ Full | ⚠️ Tier only | ❌ N/A |
...
| Atomic writes | ❌ | ✅ | ✅ | N/A |
| Dedicated install doc | ✅ PI-DEV-RENDERER.md | ✅ OPENCODE-INSTALL.md | ❌ | ❌ |

### 6.2 Compliance Matrix

| Requirement | π.dev | OpenCode | Claude Code | Copilot CLI |
|-------------|-------|----------|-------------|-------------|
| Canonical 8 roles | ❌ (9 roles) | ✅ | ✅ | N/A |
| Current model IDs | ❌ (2024 IDs) | ✅ | ⚠️ (tiers) | N/A |
| SPEC.md layout | ⚠️ | ✅ | ✅ | ✅ |
...
| File | Status |
|------|--------|
| `renderer/PI-DEV-RENDERER.md` | ⚠️ Partially stale |
| `docs/OPENCODE-INSTALL.md` | ✅ Thorough |
| `docs/INSTALL.md` | ❌ Stale (wrong script paths) |
| Claude Code install guide | ❌ Missing |
| Copilot CLI install guide | ❌ Missing |
