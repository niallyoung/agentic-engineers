...
/quality-gate-orchestration {workspace-root}/{service-name} --deployment-target prod

# Faster check (skip expensive E2E)
/quality-gate-orchestration {workspace-root}/{service-name} --skip-e2e

# With state migration validation
/quality-gate-orchestration {workspace-root}/{service-name} --deployment-target prod --validate-migrations

# JSON output for CI/CD
/quality-gate-orchestration {workspace-root}/{service-name} --json --output-dir ./quality-reports/
```
...
```json
{
  "service_path": "{workspace-root}/{service-name}",
...
```json
{
  ...
  "report_location": "{workspace-root}/quality-reports/{service-name}/2026-04-28-08-15/report.json"
}
```
...
```bash
$ /quality-gate-orchestration {workspace-root}/{service-name} --deployment-target prod --json
...
Report saved: {workspace-root}/quality-reports/{service-name}/2026-04-28-08-15/report.json
```
