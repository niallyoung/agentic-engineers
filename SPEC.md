...
status: Superseded — see docs/SPEC.md
type: specification
---

# Agentic Engineers Implementation Specification

> ⚠️ **NON-CANONICAL COPY.** The authoritative specification is
> [`docs/SPEC.md`](docs/SPEC.md) — it is the file protected by the
> spec-management skill, validated by spec-validator and the git hooks, and
> deployed by the render pipeline. This root copy has drifted (last
> consolidated 2026-05-02) and is retained only because some of its LOCKED
> sections (e.g. *Approved Claude Models*, *Model Naming Architecture*) have
> not yet been merged into `docs/SPEC.md` via the spec-management proposal
> workflow. **Do not edit this file**; propose changes against
> `docs/SPEC.md` instead.
