...
Auto-sync queue DELEGATEs ↔ TODO.md on task lifecycle events. Maintains TODO.md as a single source of truth for task tracking.

### Features

1. **DELEGATE → TODO.md sync** — Automatically add new tasks to TODO.md when DELEGATEs are created in queue/incoming/
2. **HANDBACK → TODO.md sync** — Automatically mark tasks complete when HANDBACKs are received in queue/done/
3. **Bidirectional sync** — Detect and resolve conflicts between TODO.md and queue with TODO-first merge strategy
4. **Orphan detection** — Flag tasks in TODO but not in queue (manual entries, deleted queue items)
5. **Missing detection** — Flag tasks in queue but not in TODO (new DELEGATEs not yet synced)
6. **Weekly reporting** — Generate sync reports with metrics, completion rates, and recommendations
7. **CLI wrapper** — `opencode-todo-sync` command for manual invocation

### Invocation

```bash
# Automatic (via Orchestrator post-DELEGATE, post-HANDBACK)
...
3. Applies merge strategy: TODO.md manual edits take precedence
4. Generates sync report with recommendations

### Conflict Resolution Strategy

**TODO.md-first merge strategy:**
...
This ensures manual edits to TODO.md are not overwritten by automated syncs.
...
- Post-DELEGATE: `manager.sync_delegate_to_todo(delegate_data)`
- Post-HANDBACK: `manager.sync_handback_to_todo(handback_data)`

**CLI Integration:**
- `opencode-todo-sync sync` — Full bidirectional sync
...
- `DelegateEntry` — Parse and format DELEGATE entries
- `HandbackEntry` — Parse and format HANDBACK entries
- `TodoSyncManager` — Main synchronization manager
- `SyncConflict` — Represents a sync conflict
- `SyncReport` — Weekly sync report

**Methods:**

- `sync_delegate_to_todo()` — Add DELEGATE to TODO.md
- `sync_handback_to_todo()` — Mark HANDBACK as complete
- `detect_orphaned_tasks()` — Find tasks in TODO but not in queue
- `detect_missing_tasks()` — Find tasks in queue but not in TODO
- `detect_conflicts()` — Find conflicting versions
- `bidirectional_sync()` — Full sync with all checks
- `generate_weekly_report()` — Create weekly report
...
- `0` — Sync successful, no issues
- `1` — Sync complete but issues detected (conflicts, orphans, missing)
- `2` — Error (bad arguments, invalid path, file I/O error)
