...
| ID | Task | Why Now | Effort | Extends |
|---|---|---|---|---|
| 1 | Publish DELEGATE/HANDBACK as standalone spec page | No open delegation standard; first-mover text costs nothing | S | `docs/specs/protocol-core-v1.0.yaml` |
| 2 | agentskills.io conformance audit + badge | 32 tools read SKILL.md; conformance = free portability | S | `src/skills/` |
| 3 | AGENTS.md v1.0 readiness | AAIF stable spec + validation tooling 2026 roadmap; CI step stub | S-M | `renderer/`, `pre-push` |
| 4 | Subagent cost guardrail | Runaway-cost is loudest pain in niche; our metrics fields exist | M | `PROTOCOL.md`, orchestrator skill |
...
| 6 | /fleet-aware Copilot rendering | Map Orchestrator fan-out onto Copilot's native `/fleet` command (GA Apr 2026) | S-M | `renderer/` copilot path |
...
| 10 | HANDBACK cost rollup report | Session-transcript-derived cost/quality summary per role; #1 asked-for capability | M | Post-session analysis |
