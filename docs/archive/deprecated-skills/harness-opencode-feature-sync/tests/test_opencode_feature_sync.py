...
# ---------------------------------------------------------------------------
# Path bootstrap (mirrors src/skills/file-sync/tests style)
# ---------------------------------------------------------------------------
_SKILL_ROOT = Path(__file__).parent.parent
sys.path.insert(0, str(_SKILL_ROOT))

from scripts.opencode_feature_sync import (  # noqa: E402
    build_parser,
    build_report,
    detect_drift,
...
_REGISTRY = _SKILL_ROOT / "references" / "integration-points.yaml"
...
def test_update_registry_idempotent_candidates(tmp_path: Path):
    reg_path = tmp_path / "registry.yaml"
    registry = {"integration_points": []}
    write_registry(registry, reg_path)
    cand = [{"id": "c1", "opencode_path": "a.ts", "anchor": "", "what_to_check": "x",
             "last_verified": "2026-05-31", "status": "candidate"}]
    update_registry(registry, [], cand, set(), reg_path)
    update_registry(registry, [], cand, set(), reg_path)
    reloaded = load_registry(reg_path)
    assert sum(1 for e in reloaded["integration_points"] if e["id"] == "c1") == 1
...
def test_build_report_contains_sections():
    report = build_report({"model"}, {"model", "thinking"}, [], [], [])
    assert "# OpenCode Feature Sync Report" in report
    assert "Drift findings" in report


def test_parser_defaults_read_only():
    args = build_parser().parse_args([])
    assert args.update_registry is False
