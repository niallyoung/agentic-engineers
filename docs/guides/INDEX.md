# Guides Index

Practical how-to guides for working with the agentic-engineers framework. For the
canonical protocol/architecture reference, start at [docs/INDEX.md](../INDEX.md)
instead — this page only catalogs the `docs/guides/` directory.

| Guide | Purpose |
|-------|---------|
| [agent-creation.md](agent-creation.md) | How to create a new agent role |
| [skill-creation.md](skill-creation.md) | How to create a new skill |
| [claude-harness-extension.md](claude-harness-extension.md) | Extending the Claude Code harness |
| [deployment.md](deployment.md) | Deploying/installing the framework into a harness |
| [thinking-modes-and-cost-quality-trade-offs.md](thinking-modes-and-cost-quality-trade-offs.md) | When to enable extended thinking, and its cost impact |
| [troubleshooting.md](troubleshooting.md) | Common problems under the current direct-dispatch architecture |
| [harness-setup/](harness-setup/) | Per-harness setup instructions (Claude, Codex, OpenCode, etc.) |
| [examples/](examples/) | Real DELEGATE/HANDBACK examples |

See also [docs/guides/README.md](README.md) for a shorter orientation to this directory.
