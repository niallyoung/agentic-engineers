...
class TestQueueStateTransitions(unittest.TestCase):
    ...
    def test_move_task_from_incoming_to_processing(self):
        """Test: Move task from incoming to processing after delegation."""
        task_id = "2026-05-02-test-task"
        
        # Create task in incoming
        incoming_file = self.queue_dir / "incoming" / f"{task_id}.yaml"
        task_data = {"task_id": task_id, "description": "Test"}
        ...
        # Verify: task no longer in incoming, is in processing
        self.assertFalse(incoming_file.exists())
        processing_file = self.queue_dir / "processing" / f"{task_id}-DELEGATE-engineer.yaml"
        # After implementation, should exist

    def test_move_task_from_processing_to_done(self):
        """Test: Move task from processing to done after QE review."""
        task_id = "2026-05-02-test-task"
        
        # Create HANDBACK in processing
        processing_file = self.queue_dir / "processing" / f"{task_id}-HANDBACK-engineer.yaml"
        handback_data = {
            "handoff_type": "HANDBACK",
            "task_id": task_id,
            "status": "complete"
        ...
        # Verify: task no longer in processing, is in done
        self.assertFalse(processing_file.exists())
        done_file = self.queue_dir / "done" / f"{task_id}-PROCEED.yaml"
        # After implementation, should exist
