...
class TestEnumDriftDetection:
    """Tests for scan_status_enum_drift function."""

    def test_enum_drift_report_structure(self):
        """EnumDriftReport has expected attributes."""
        from protocol_validator import EnumDriftReport, EnumDriftFinding
        ...
        report = EnumDriftReport(
            canonical={"success", "failure", "partial", "blocked", "escalate"},
            legacy_aliases={"complete", "failed"},
            findings=findings,
        )
        assert report.has_drift is True
        assert len(report.findings) == 1

    def test_enum_drift_no_drift_report(self):
        """EnumDriftReport with no findings shows no drift."""
        from protocol_validator import EnumDriftReport
        ...
        assert "No HANDBACK status enum drift" in report.to_text()

    def test_enum_drift_to_text_formatting(self):
        """EnumDriftReport.to_text() formats findings properly."""
        from protocol_validator import EnumDriftReport, EnumDriftFinding
...
class TestProtocolDivergenceDetection:
    """Tests for scan_protocol_divergence function."""

    def test_protocol_divergence_report_structure(self):
        """ProtocolDivergenceReport has expected attributes."""
        from protocol_validator import ProtocolDivergenceReport, ProtocolDivergence

        findings = [
            ProtocolDivergence(
                location="src/custom/escalation.py",
                description="Custom escalation handler (should use orchestrator)",
                severity="warning",
            )
        ]
        report = ProtocolDivergenceReport(findings=findings)
        assert report.has_issues is True
        assert len(report.findings) == 1

    def test_protocol_divergence_no_issues(self):
        """ProtocolDivergenceReport with no findings shows no divergence."""
        from protocol_validator import ProtocolDivergenceReport

        report = ProtocolDivergenceReport(findings=[])
        assert report.has_issues is False
        assert "No protocol divergence" in report.to_text()

    def test_protocol_divergence_to_text_formatting(self):
        """ProtocolDivergenceReport.to_text() formats findings properly."""
        from protocol_validator import ProtocolDivergenceReport, ProtocolDivergence

        divergence = ProtocolDivergence(
            location="src/agents/custom_escalator.py",
            description="Custom escalation handler (should use orchestrator polling)",
            severity="warning",
        )
        report = ProtocolDivergenceReport(findings=[divergence])
        text = report.to_text()
        assert "custom_escalator.py" in text
