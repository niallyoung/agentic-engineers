...
# ---------------------------------------------------------------------------
# 4. Git push helpers — delegate to shell tests
# ---------------------------------------------------------------------------


class TestGitPushHelpers:
    """Delegate to the shell test suite for git_push.sh."""

    git_push_sh = REPO_ROOT / "src" / "skills" / "_meta" / "git-operations" / "scripts" / "git_push.sh"
    shell_test = REPO_ROOT / "src" / "skills" / "_meta" / "git-operations" / "tests" / "test_git_push.sh"

    def test_git_push_sh_exists(self):
        assert self.git_push_sh.exists(), "git_push.sh helper script must exist"

    def test_shell_test_file_exists(self):
        assert self.shell_test.exists(), "test_git_push.sh must exist"

    def test_git_push_sh_syntax(self):
        """Verify git_push.sh has valid bash syntax."""
        result = subprocess.run(
            ["bash", "-n", str(self.git_push_sh)],
            capture_output=True,
            text=True,
        )
        assert result.returncode == 0, f"bash -n failed:\n{result.stderr}"

    def test_git_push_shell_tests_pass(self):
        """Run the shell test suite; skip if bash is unavailable."""
        bash = shutil.which("bash")
        if not bash:
            pytest.skip("bash not available")

        result = subprocess.run(
            [bash, str(self.shell_test)],
            capture_output=True,
            text=True,
            cwd=str(REPO_ROOT),
        )
        assert result.returncode == 0, (
            f"test_git_push.sh failed:\nSTDOUT:\n{result.stdout}\nSTDERR:\n{result.stderr}"
        )

    def test_git_push_sh_exports_push_function(self):
        """Sourcing git_push.sh makes git_push_with_tags available."""
        result = subprocess.run(
            ["bash", "-c", f"source {self.git_push_sh} && type git_push_with_tags"],
            capture_output=True,
            text=True,
        )
        assert result.returncode == 0, "git_push_with_tags function not found after sourcing"

    def test_git_push_sh_exports_validate_function(self):
        """Sourcing git_push.sh makes git_validate_tags available."""
        result = subprocess.run(
            ["bash", "-c", f"source {self.git_push_sh} && type git_validate_tags"],
            capture_output=True,
            text=True,
        )
        assert result.returncode == 0, "git_validate_tags function not found after sourcing"

    def test_dry_run_does_not_push(self):
        """--dry-run flag produces output but does not execute any push."""
        # Create a temp repo with no remote configured — push would fail
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = subprocess.run(
                [
                    "bash",
                    "-c",
                    f"cd {repo} && source {self.git_push_sh} && git_push_with_tags --dry-run",
                ],
                capture_output=True,
                text=True,
            )
