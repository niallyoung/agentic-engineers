...
class TestCommitMsgHook:
    ...
    def test_error_message_shows_char_count(self):
        """Short message error must include the actual character count."""
        result = self._run_with_msg("short")
        assert result.returncode != 0
...
class TestPrePushHook:
    ...
    def test_summary_shows_check_counts(self):
        """Summary section must show checks passed/failed counts."""
        with tempfile.TemporaryDirectory() as tmpdir:
            make_minimal_repo_with_docs(tmpdir)
            result = self._run_push_in_repo(tmpdir)
        combined = result.stdout + result.stderr
        assert "passed" in combined.lower() or "Checks" in combined
...
class TestLegacyPreCommitHook:
    """Harness D: .git/hooks/pre-commit — protocol DELEGATE/HANDBACK validation."""

    HOOK = GIT_HOOKS_DIR / "pre-commit"

    # ── Bypass ─────────────────────────────────────────────────────────────────

    def test_bypass_hook_validation_env_exits_zero(self):
        """BYPASS_HOOK_VALIDATION=true must exit 0 immediately."""
        result = run_hook(self.HOOK, env_overrides={"BYPASS_HOOK_VALIDATION": "true"})
        assert result.returncode == 0

    def test_bypass_false_does_not_bypass(self):
        """BYPASS_HOOK_VALIDATION=false must NOT bypass."""
        with tempfile.TemporaryDirectory() as tmpdir:
            make_git_repo(tmpdir)
            result = run_hook_in_repo(
                self.HOOK, tmpdir,
                env_overrides={"BYPASS_HOOK_VALIDATION": "false"},
            )
        assert result.returncode == 0  # No staged files → passes normally

    # ── SPEC constraint checks ─────────────────────────────────────────────────

    def test_rejects_py_in_orchestration_scripts(self):
        """Legacy hook must reject .py in orchestration/scripts/."""
        with tempfile.TemporaryDirectory() as tmpdir:
            make_git_repo(tmpdir)
            stage_file(tmpdir, "orchestration/scripts/bad.py", "x=1\n")
            result = run_hook_in_repo(
    ...
    def test_rejects_cron_in_orchestration_config(self):
        """Legacy hook must reject .cron in orchestration/config/."""
        with tempfile.TemporaryDirectory() as tmpdir:
            make_git_repo(tmpdir)
            stage_file(tmpdir, "orchestration/config/sched.cron", "* * * * * /bin/true\n")
            result = run_hook_in_repo(
                self.HOOK, tmpdir,
                env_overrides={"BYPASS_HOOK_VALIDATION": ""},
            )
    ...
    def test_spec_error_message_references_docs(self):
        """SPEC violation error must reference docs/SPEC.md."""
        with tempfile.TemporaryDirectory() as tmpdir:
            make_git_repo(tmpdir)
            stage_file(tmpdir, "orchestration/scripts/bad.py", "x=1\n")
            result = run_hook_in_repo(
        ...
        assert result.returncode != 0
        combined = result.stdout + result.stderr
        assert "SPEC" in combined or "docs" in combined.lower()
    ...
    def test_error_message_includes_bypass_hint(self):
        ...
        assert result.returncode != 0
        combined = result.stdout + result.stderr
        assert "BYPASS_HOOK_VALIDATION" in combined or "bypass" in combined.lower()

    def test_validation_passed_message_on_success(self):
        """On success with DELEGATE/HANDBACK files, hook must print validation passed."""
        with tempfile.TemporaryDirectory() as tmpdir:
            make_git_repo(tmpdir)
            stage_file(tmpdir, "delegate.yaml", textwrap.dedent("""\
                handoff_type: DELEGATE
        ...
        assert result.returncode == 0
        combined = result.stdout + result.stderr
        assert "passed" in combined.lower() or "\u2713" in combined or "\u2705" in combined

    # ── Non-YAML files ignored ─────────────────────────────────────────────────

    def test_non_yaml_files_not_validated(self):
        """Non-.yaml/.yml files must not be validated as DELEGATE/HANDBACK."""
        with tempfile.TemporaryDirectory() as tmpdir:
            make_git_repo(tmpdir)
            stage_file(tmpdir, "notes.txt", "DELEGATE\ntask_id: bad-id\n")
            result = run_hook_in_repo(
                self.HOOK, tmpdir,
                env_overrides={"BYPASS_HOOK_VALIDATION": ""},
            )
...
class TestHookInstallation:
    ...
    def test_hook_has_shebang(self, hook_path):
        """Each hook must start with a shebang line."""
        content = hook_path.read_text()
        assert content.startswith("#!"), "Missing shebang in: {}".format(hook_path)

    @pytest.mark.parametrize("hook_path", [
        GITHOOKS_DIR / "pre-commit",
        GITHOOKS_DIR / "commit-msg",
        GITHOOKS_DIR / "pre-push",
    ])
    def test_githooks_have_install_comment(self, hook_path):
        """Each .githooks/ hook must document its install command."""
        content = hook_path.read_text()
        assert "Install" in content or "hooksPath" in content or "git config" in content, \
            "No install instructions in: {}".format(hook_path)

    @pytest.mark.parametrize("hook_path", ALL_HOOK_PATHS)
    def test_all_hooks_have_bypass_mechanism(self, hook_path):
        """Every hook must document a bypass mechanism."""
        content = hook_path.read_text()
        has_bypass = (
            "SKIP_HOOKS" in content
            or "BYPASS_HOOK_VALIDATION" in content
            or "SKIP_COMMIT_MSG_HOOK" in content
        )
        assert has_bypass, "No bypass mechanism documented in: {}".format(hook_path)
