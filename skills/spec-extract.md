...
### Input
- **Service path**: `/path/to/service` (e.g., `{workspace-root}/{service-name}`)
- **Optional flags**:
...
/spec-extract {workspace-root}/{service-name}

# Scan with output to central location
/spec-extract {workspace-root}/{service-name} \
  --output-dir {workspace-root}/{service-name}/specs
```
...
/spec-extract {workspace-root}/{service-name} --json > spec-report.json
...
for service in {service-name} {service-name} {service-name} {service-name} {service-name} {service-name} {service-name}; do
  /spec-extract {workspace-root}/$service \
    --output-dir {workspace-root}/{service-name}/specs
done
```
...
Spec files written to: {workspace-root}/{service-name}/specs/
