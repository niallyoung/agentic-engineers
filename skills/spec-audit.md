...
/spec-audit $WORKSPACE_ROOT/{example-service}

# With options
/spec-audit $WORKSPACE_ROOT/{example-service} --fail-on-critical --json
```
...
for service in {example-service} {example-service} {example-service} {service-name} {service-name} {example-service} {service-name} {service-name}; do
  ...
  /spec-audit $WORKSPACE_ROOT/$service --output-dir $WORKSPACE_ROOT/{workspace-name}/audits
done
```
