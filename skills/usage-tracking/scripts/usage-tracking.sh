...
Or every hour:
  ...
  0 * * * * $WORKSPACE_ROOT/{workspace-name}/agentic-engineers/orchestration/scripts/capture_token_usage.sh

Install with:
  (crontab -l 2>/dev/null; echo "*/30 * * * * $WORKSPACE_ROOT/{workspace-name}/agentic-engineers/orchestration/scripts/capture_token_usage.sh") | crontab -
