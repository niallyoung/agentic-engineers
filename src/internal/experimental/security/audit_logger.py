"""
DEPRECATED: Audit Logger — Immutable audit trail for all DELEGATE/HANDBACK transitions.

LOCATION: src/internal/experimental/security/audit_logger.py
STATUS: Superseded by session_manager (src/orchestration/memory/session_manager.py)

Records all state transitions with timestamps, agents, and outcomes for
compliance, debugging, and security auditing.

This module has been superseded by the session memory system, which provides
structured audit trails and task lifecycle tracking. However, it is preserved
for reference and potential future protocol hardening phases.

Current audit trail implementation: src/orchestration/memory/session_manager.py

To re-integrate or migrate:
1. Review session_manager.py for equivalent audit functionality
2. Migrate any missing event types to session memory system
3. Consider merging with security audit events
