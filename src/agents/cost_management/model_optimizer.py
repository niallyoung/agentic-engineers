...
Usage:
    ...
    # Get recommendations
    cheapest = optimizer.recommend_cheapest(
        task_type="general", input_tokens=1000, output_tokens=500)
    best_quality = optimizer.recommend_best_quality(
        task_type="code_review", input_tokens=5000, output_tokens=2000)
    balanced = optimizer.recommend_balanced(
        task_type="general", input_tokens=1000, output_tokens=500)
...
class ModelOptimizer:
    ...
    def _ensure_selector(self) -> Any:
        ...
        if self._selector is None:
            try:
                ...
                logger.error("Failed to load ModelSelector: %s", exc)
                raise RuntimeError(
                    "ModelSelector required but not available") from exc
    ...
    def recommend_cheapest(
        ...
        Args:
            ...
            constraints: Optional constraints dict (unused)
        ...
        # Recommend cheapest by finding minimum cost model
    ...
    def recommend_fastest(
        ...
        return RecommendationResult(
            ...
            reasoning=f"Selected fastest model: {fastest['estimated_latency_sec']:.2f}s",
    ...
    def recommend_best_quality(
        ...
        return RecommendationResult(
            ...
            reasoning=f"Selected best quality: {best['estimated_quality']:.2f}",
    ...
    def recommend_balanced(
        ...
        for model in models:
            cost_norm = 1.0 - (model["estimated_cost"] / max_cost) \
                if max_cost > 0 else 1.0
            quality_norm = model["estimated_quality"]
            latency_norm = 1.0 - (model["estimated_latency_sec"] / max_latency) \
        ...
        return RecommendationResult(
            ...
            reasoning=(f"Balanced with weights: cost={cost_weight:.2f}, "
    ...
    def simulate_model_mix(
        ...
        for model_name, fraction in mix.items():
            ...
            latency = selector._quality.get_avg_latency(model_name) / 1000.0
