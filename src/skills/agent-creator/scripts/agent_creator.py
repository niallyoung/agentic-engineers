...
# Allowed creation types (merged from skill-creator skill — Wave 3 consolidation)
ALLOWED_TYPES: List[str] = ["agent", "skill"]
...
When you use **{config.name}** during a task, include a `skill_feedback` entry
...
# ============================================================================
# SKILL SCAFFOLD (merged from skill-creator — Wave 3 consolidation)
# ============================================================================

@dataclass
class SkillConfig:
    """Configuration for scaffolding a new skill (agentskills.io spec format).

    Absorbed from skill-creator skill during Wave 3 consolidation (m3-skills-deprecation).
    agent-creator now handles both --type agent (default) and --type skill.
    ...
    Optional:
        category    — Category tag (default: "utility")
        role        — Role hint in SKILL.md (default: "orchestrator")
        model       — Model hint (default: "claude-haiku-4.5")
...
class SkillScaffoldGenerator:
    """Generates scaffold files for a new agentskills.io-spec skill.

    Produces:
      - SKILL.md (with frontmatter following the framework pattern)
      - scripts/.gitkeep (placeholder for skill scripts)
      - tests/test_<skill>.py (minimal import test)
      - references/REFERENCE.md (documentation placeholder)
    """

    def generate_skill_md(self, config: SkillConfig) -> str:
        """Generate SKILL.md for a new skill."""
        description = config.description or f"{config.name} skill."
        return (
            "---\n"
            f"name: {config.name}\n"
            f"description: {description}\n"
            ...
            f"    └── test_{config.name.replace('-', '_')}.py\n"
            "```\n\n"
            "## Usage\n\n"
            "TODO: describe how to invoke this skill.\n\n"
            "## Implementation Notes\n\n"
            "- TODO: describe key logic\n"
            "- TODO: document external dependencies\n"
        )

    def generate_test_scaffold(self, config: SkillConfig) -> str:
        """Generate minimal test scaffold for the new skill."""
        module_name = config.name.replace("-", "_")
        return (
            f'"""Basic import tests for {config.name} skill.\n\n'
            f'Generated by agent-creator --type skill (Wave 3 consolidation).\n'
            f'"""\n'
            f'from pathlib import Path\n\n\n'
            f'def test_skill_directory_exists():\n'
            f'    """Skill directory should exist with SKILL.md."""\n'
            f'    skill_dir = Path(__file__).parent.parent\n'
            f'    assert skill_dir.exists()\n'
            f'    assert (skill_dir / "SKILL.md").exists()\n\n\n'
            f'def test_scripts_directory_exists():\n'
            f'    """Scripts directory should exist."""\n'
            f'    scripts_dir = Path(__file__).parent.parent / "scripts"\n'
            f'    assert scripts_dir.exists()\n\n\n'
            f'def test_references_directory_exists():\n'
            f'    """References directory should exist."""\n'
            f'    refs_dir = Path(__file__).parent.parent / "references"\n'
            f'    assert refs_dir.exists()\n'
        )

    def generate_reference_md(self, config: SkillConfig) -> str:
        """Generate placeholder references/REFERENCE.md."""
        return (
            f"# {config.name} — Reference\n\n"
    ...
    def plan_files(self, config: SkillConfig, skill_dir: Path) -> Dict[Path, str]:
        """Return {path: content} dict for all files to create."""
        module_name = config.name.replace("-", "_")
        return {
            skill_dir / "SKILL.md": self.generate_skill_md(config),
            skill_dir / "scripts" / ".gitkeep": "",
            skill_dir / "tests" / f"test_{module_name}.py": self.generate_test_scaffold(config),
            skill_dir / "references" / "REFERENCE.md": self.generate_reference_md(config),
        }
...
def create_skill(
    ...
    Args:
        config:      SkillConfig describing the new skill.
        output_root: Base directory for generated skills (default: src/skills/).
        dry_run:     If True, validate and plan but do NOT write files.

    Returns:
        CreationResult with status SUCCESS, DRY_RUN, or FAILED.
    """
    ...
    # Validate name using ConfigValidator
    validator = ConfigValidator()
    name_errors = validator.validate_name(config.name)
    if name_errors:
        return CreationResult(
            status=CreationStatus.FAILED,
            agent_name=config.name,
            errors=name_errors,
            span={"files_created": 0, "duration_ms": 0},
        )

    generator = SkillScaffoldGenerator()
    skill_dir = output_root / config.name
    planned = generator.plan_files(config, skill_dir)
    deliverables = [str(p) for p in planned.keys()]
    ...
    files_created = 0 if dry_run else len(planned)
    duration_ms = int((_time.monotonic() - t_start) * 1000)
    status = CreationStatus.DRY_RUN if dry_run else CreationStatus.SUCCESS

    return CreationResult(
        status=status,
        agent_name=config.name,
        deliverables=deliverables,
        span={"files_created": files_created, "duration_ms": duration_ms},
    )
...
def _main(argv: Optional[List[str]] = None) -> int:  # pragma: no cover
    ...
    parser.add_argument("--type", default="agent", choices=ALLOWED_TYPES,
                        help="Creation type: 'agent' (default) or 'skill' (agentskills.io spec)")
    parser.add_argument("--role", required=False, choices=ALLOWED_ROLES, help="Agent role (required for --type agent)")
    ...
    # Route to skill scaffold if --type skill
    if args.type == "skill":
        ...
        result = create_skill(skill_config, output_root=output_root, dry_run=args.dry_run)
    else:
        # Default: agent scaffold (--type agent)
        if not args.role:
            print("ERROR: --role is required for --type agent", file=sys.stderr)
            return 1
        config = AgentConfig(
            name=args.name,
            role=args.role,
